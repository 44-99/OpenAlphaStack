...
Importable from paper_engine.py or any other engine component.
"""
...
def notify_overnight_complete(run_id: str, summary: dict) -> None:
    """Overnight pipeline finished. summary keys: bias, candidates, passed, rejected, nav."""
    if not _chat_ids():
        return
...
def notify_overnight_timeout(run_id: str, stage: str) -> None:
    ...
    if not _chat_ids():
        return
    msg = f"⚠️ 盘后流水线超时 — {stage} 阶段未在时限内完成\nRun: {run_id}"
    _send_all(msg)
...
def notify_trade(
    ...
    lines = [f"{action_label} — {symbol} {name}"]
    if data_time:
