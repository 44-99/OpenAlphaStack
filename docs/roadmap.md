...
| ~~P1~~ | ~~回复冗长度模式~~ | cc-connect | ❌ 已移除 | 单用户场景过度设计；需要简短回复时直接告诉 AI 即可 |
| ~~P1~~ | ~~飞书确认卡片~~ | cc-connect | ❌ 已移除 | 实盘确认用纯文本消息即足够；卡片交互增加复杂度无安全增益 |
| ~~P1~~ | ~~轻量监控面板~~ | cc-connect / FinceptTerminal | ❌ 已移除 | `/status` `/持仓` `/交易` 等文本命令已覆盖相同信息 |
| ~~P2~~ | ~~工具注册元数据~~ | Vibe-Trading | ❌ 已移除 | 19 个工具手动维护表格每月仅需 30 秒，自动生成 ROI 为负 |
...
| P0 | 人工确认流 | 大额或所有实盘订单必须走飞书消息确认（纯文本 Y/N 二次确认） |
