...
def resolve_universe(spec: str) -> list[str]:
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "alphaclaude.tools.screen", "-s", strategy, "--list"],
    ...
    try:
        result = subprocess.run(
            [sys.executable, "-m", "alphaclaude.tools.screen", "-s", strategy],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="AlphaClaude Backtest Runner — package engine wrapper")
