...
class AgentTaskRunner:
    ...
    def run_postclose_review(self, review_context: str = "") -> AgentTaskResult:
        task_id = "postclose_review"
        artifacts_dir = self.output_dir / "agent_runs" / task_id
        artifacts_dir.mkdir(parents=True, exist_ok=True)
        prompt = self._build_postclose_prompt(
            artifacts_dir=artifacts_dir,
...
"""
    ...
    def _build_postclose_prompt(self, artifacts_dir: Path, review_context: str) -> str:
        return f"""你是 AlphaClaude 盘后复盘主 Agent。

任务目标：
1. 先阅读 `CLAUDE.md`，遵守项目准则。
2. 阅读 `skills/README.md`，再按复盘任务需要自行选择并阅读 `skills/` 下的 `SKILL.md` 和 references。
3. 自主启动或模拟必要的子 Agent / 子任务，分析今日交易、信号质量、亏损来源和可改进策略。
4. 只做复盘和归因，不生成新的交易指令，不修改 `plan.json`、`state.json` 或 `ledger.jsonl`。

硬约束：
- 不要执行真实或模拟交易。
- 不要修改源代码。
- 只允许把复盘产物写入：`{artifacts_dir}`
- 必须把无法获取的数据标记为缺失，不要编造。

审计协议（必须遵守）：
- 如果你启动任何子任务或子 Agent，都必须用 `agent_event` CLI 留痕。
- 子任务开始前，先把输入写入 `{artifacts_dir}/tasks/<task_id>/input.md`，然后执行：
  `python -m alphaclaude.engine.agent_event start --run-dir "{artifacts_dir}" --task-id "<task_id>" --parent-task-id "postclose_review" --role "<角色>" --summary "<开始摘要>" --input-ref "tasks/<task_id>/input.md"`
- 子任务完成后，把输出写入 `{artifacts_dir}/tasks/<task_id>/output.md`，结构化摘要写入 `{artifacts_dir}/tasks/<task_id>/result.json`，然后执行：
  `python -m alphaclaude.engine.agent_event finish --run-dir "{artifacts_dir}" --task-id "<task_id>" --parent-task-id "postclose_review" --role "<角色>" --status "success" --summary "<完成摘要>" --output-ref "tasks/<task_id>/output.md" --result-ref "tasks/<task_id>/result.json"`
- 如果子任务失败，也必须执行 finish，并使用 `--status "error"` 与 `--error "<错误原因>"`。
- 你可以自由决定 task_id、role、子任务数量和执行顺序；Python 只审计事件，不预设你的子 Agent。

必须产出以下文件：
- `review_report.md`: 中文盘后复盘报告，包含今日执行、盈亏、错因、风险、明日关注点。
- `strategy_attribution.json`: 结构化归因，字段至少包含 `summary`、`trades`、`signal_quality`、`risk_issues`、`improvement_actions`。
...
可用复盘上下文：
```text
{review_context or "(未提供)"}
```
"""

    def _load_json_artifacts(self, artifacts_dir: Path) -> dict[str, Any]:
        artifacts: dict[str, Any] = {}
        for name in ("candidate_evidence", "plan_draft", "strategy_attribution"):
