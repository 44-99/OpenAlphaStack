...
from openalphastack.engine.state import EngineState, round_lot
...
class FastLane:
    ...
    It never calls an Agent or model. Emergency conditions are logged and
    surfaced to the operator; hard rules remain authoritative.
    """
    ...
    def __init__(self, state: EngineState, plan: PlanManager,
        ...
        # The full universe is the research selection pool, not scanned every tick.
        self._monitored = set()
    ...
    def _load_t0_configs(self) -> None:
        ...
        if not found_any:
            ...
            for code, h in self.state.holdings.items():
                ...
                # Default parameters per bucket (from t0-intraday skill)
    ...
    def tick(self, minute_ts: pd.Timestamp | None = None,
        ...
        if self.data_feed and self.mode == "backtest":
            ...
            def _fetch_one(code):
                try:
                    from openalphastack.tools._fallback import get_quote
                    q, _ = get_quote(code)
            ...
            try:
                from openalphastack.tools.quote import get_market_overview
        ...
        try:
            from openalphastack.tools.signal_rules import scan_code
