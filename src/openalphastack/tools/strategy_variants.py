...
def get_active_variant(force: str = "") -> dict:
    """Classify current market state and select a deterministic variant.

    MCP tools and external research workflows may call this helper before plan
    construction. It never raises and returns the default variant on failure.
    """
    try:
...
def variant_bc_rules_text(variant: dict) -> str:
    """Build the B/C rules string for external-Agent prompts from variant config."""
