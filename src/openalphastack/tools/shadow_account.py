...
Pure Python computes the numbers; an external Agent may interpret the patterns.
"""
import argparse
import json
import os
...
# ═══════════════════════════════════════════════════════════════
# Deferred reflection handoff (TradingAgents-inspired).
# Python computes diagnostics; an external Agent can consume the generated
# prompt and decide whether the reflection should influence a later plan.
# ═══════════════════════════════════════════════════════════════


def build_reflection_prompt(diagnostics: dict) -> str:
    """Build a prompt for an external Agent to reflect on trading patterns.
    ...
    prompt = (
        ...
        f"已完成{s.get('win_rate', 0)}%胜率共{diagnostics['paired_trades_count']}笔交易，"
        f"累计盈亏{float(s.get('total_pnl', 0) or 0):,.0f}元。"
    )
    detail_keys = {
        "avg_winner_hold_days", "avg_winner_pnl_pct",
        "avg_loser_hold_days", "avg_loser_pnl_pct",
    }
    if detail_keys.issubset(s):
        prompt += (
            f"赢家均持{s['avg_winner_hold_days']}天/均利{s['avg_winner_pnl_pct']}%，"
            f"输家均持{s['avg_loser_hold_days']}天/均亏{abs(s['avg_loser_pnl_pct'])}%。"
        )
...
def format_reflections_for_prompt(reflection_text: str) -> str:
    """Format reflections for inclusion in a later external-Agent prompt.
...
def prepare_reflection_prompt(run_id: str) -> str:
    """Return a reflection prompt for an external Agent without invoking one."""
    diag = load_latest_diagnostics(run_id)
    if not diag or diag.get("paired_trades_count", 0) < 4:
        return ""
    return build_reflection_prompt(diag)
...
def main():
    ...
    parser.add_argument("--reflection-prompt", action="store_true",
                        help="Print a reflection prompt for an external Agent")
    args = parser.parse_args()

    if args.reflection_prompt:
        prompt = prepare_reflection_prompt(args.run_id)
        if prompt:
            print(prompt)
        else:
            print("(insufficient data for reflection — need >= 4 paired trades)")
        return
