...
def _is_engine_command(line: str) -> bool:
    return (
        "openalphastack.engine.cli" in line
        or "openalphastack app.cli engine" in line
        or "openalphastack engine" in line
        or "openalphastack\\engine\\cli.py" in line
        or "openalphastack/engine/cli.py" in line
    )
...
def format_status_text(runs: list[dict] | None = None) -> str:
    ...
    if not runs:
        return "当前没有活跃或最近的引擎实例。\n\n启动：openalphastack engine start --mode paper --capital 100000"
...
def stop_engine() -> str:
    """Kill OpenAlphaStack engine processes. Returns status message."""
